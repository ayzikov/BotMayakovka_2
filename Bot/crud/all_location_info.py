...
async def get_location_info_by_number(location_number: int):
    ...
    async with ClientSession() as session:
        async with session.get('http://127.0.0.1:8080/locations/', data=json_data) as response:
...
async def get_images_location_info_by_number(location_number: int):
    ...
    json_data = json.dumps(data)
    async with ClientSession() as session:
        async with session.get('http://127.0.0.1:8080/images/', data=json_data) as response:
